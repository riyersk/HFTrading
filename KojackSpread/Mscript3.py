...
forewards = [1,10,20,30,60,90,120,210,240,270,390,510,720]
backshift = 960
...
for stock in range(company, company+1):
    ...
    precisions = []
    noInvsts = []
    for foreward in forewards:
        test = deepcopy(data[stock])
        X = deepcopy(test)
        X['target'] = X['EndPrice'].shift(-1*(foreward-1))
        X.dropna(inplace = True)
        X['target'] = [np.log(x)-np.log(y) for x,y in zip(X['target'],X['StartPrice'])]
    ...
    with open('stock' + str(stock) + '.pkl','wb') as pfile:
