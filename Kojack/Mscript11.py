...
forewards = [[1,2,5,8,10,15,20,25,30,40,45,50,60,80,90,110,120,150],[180,210,240,270,300,330,360,390,420,450,480,510,540,600,660,720,840,960]]
backshift = 960
LSTMwidth = np.round(2*backshift, decimals = -3)
thresh = np.linspace(0,.3,num=30001)
with open('check3x1.pkl', 'rb') as picklefile:
    data = pickle.load(picklefile)
def discify(x,X):
    for i in range(1,x+1):
        X['target'] = [x+y for x,y in zip(X['target'], X['logC'].shift(-1*x))]
company = 1 #can be 0 through 9
fsec = 1 # can be 0 or 1
for stock in range(company, company+1):
    ...
    for foreward in forewards[fsec]:
        ...
        monkey = Sequential()
        monkey.add(LSTM(LSTMwidth,input_shape = (trainX2.shape[1], trainX2.shape[2]), stateful = True))
        monkey.add(Dense(2))
        monkey.add(Activation('softmax'))
        monkey.compile(loss='sparse_categorical_crossentropy', optimizer='adam', metrics=['acc'])
        monkey.fit(trainX2,np.asarray(trainy),epochs=30, validation_data=(test0X,np.asarray(test0y)))
